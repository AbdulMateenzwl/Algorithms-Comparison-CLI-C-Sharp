...
namespace Algorithms_CLI_C_sharp.DL
{
    public class Data
    {
        ...
        // Quick Sort
        public List<Org> quickSortIndex(List<Org> arr, int start, int end)
        {
            if (start < end)
            {
                Random rand = new Random();
                int pivot = start + rand.Next() % (end - start + 1);
                Org temp1 = arr[start];
                arr[start] = arr[pivot];
                arr[pivot] = temp1;
                pivot = start;
                int mid = partitionIndex(arr, start + 1, end, pivot);
                quickSortIndex(arr, start, mid - 1);
                quickSortIndex(arr, mid + 1, end);
            }
            return arr;

        }
        public int partitionIndex(List<Org> arr, int start, int end, int pivot)
        {
            int left = start;
            int right = end;
            while (left <= right)
        ...
        public List<Org> quickSortEmp(List<Org> arr, int start, int end)
        {
            if (start < end)
            {
                Random rand = new Random();
                ...
                arr[pivot] = temp1;
                pivot = start;
                int mid = partitionEmp(arr, start + 1, end, pivot);
                quickSortEmp(arr, start, mid - 1);
                quickSortEmp(arr, mid + 1, end);
            }
            return arr;

        }
        public int partitionEmp(List<Org> arr, int start, int end, int pivot)
        {
            int left = start;
            int right = end;
            while (left <= right)
            {
                while (left <= end && arr[left].no_emp < arr[pivot].no_emp)
                {
                    left++;
                }
                while (right >= start && arr[right].no_emp >= arr[pivot].no_emp)
                {
                    right--;
                }
                if (left < right)
